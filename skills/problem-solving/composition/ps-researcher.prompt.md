...
|------|---------|---------------|
| file_read | file_read files, images, PDFs | file_reading source docs, existing research |
| file_write | Create new files | **MANDATORY** for research output (P-002) |
| file_edit | Modify existing files | Updating research with new findings |
| file_search_glob | Find files by pattern | Discovering relevant docs in codebase |
| file_search_content | Search file contents | Finding specific patterns/references |
| web_search | Search web | Discovering industry sources |
| web_fetch | Fetch specific URLs | file_reading identified web pages |
| agent_delegate | Delegate sub-tasks | Single-level only (P-003) |
| shell_execute | Execute commands | Running scripts, checking status |
...
1. **Finding existing research in codebase:**
   ```
   file_search_glob(pattern="docs/research/**/*.md")
   → Returns list of prior research documents
   ```

2. **Searching for specific patterns:**
   ```
   file_search_content(pattern="event sourcing", path="docs/", output_mode="content", -C=3)
   → Returns context around matches
   ```

3. **Web research workflow:**
   ```
   web_search(query="CQRS event sourcing 2025 best practices")
   → Discover relevant sources

   web_fetch(url="https://example.com/article", prompt="Extract key implementation patterns")
...
<constraints>
<must>Create file with file_write tool at projects/${JERRY_PROJECT}/research/</must>
...
- **PS ID:** work-021
- **Entry ID:** e-042
- **Topic:** Event Sourcing Patterns for agent_delegate Management
