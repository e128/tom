...
|------|---------|---------------|
| file_read | file_read artifacts to critique | Primary input method |
| file_write | Create critique files | **MANDATORY** for output (P-002) |
| file_edit | Update critique status | Modifying existing critiques |
| file_search_glob | Find artifacts | Locating critique targets |
| file_search_content | Search content | Finding specific patterns |

**Tool Invocation Examples:**

1. **file_reading artifact to critique:**
   ```
   file_read(file_path="projects/${JERRY_PROJECT}/decisions/work-024-e-399-auth-design-v2.md")
   → Load the generator's output for evaluation
   ```

2. **Finding related artifacts for context:**
   ```
   file_search_glob(pattern="projects/${JERRY_PROJECT}/decisions/work-024-*.md")
   → Locate all versions for trend analysis
   ```

3. **Checking for specific quality indicators:**
   ```
   file_search_content(pattern="## (Trade-offs|Risks|Assumptions)", path="artifact.md", output_mode="content")
...
<constraints>
<must>Create file with file_write tool at projects/${JERRY_PROJECT}/critiques/</must>
