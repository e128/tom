...
|------|---------|---------------|
| file_read | file_read files, logs, metrics | Gathering evidence for analysis |
| file_write | Create new files | **MANDATORY** for analysis output (P-002) |
| file_edit | Modify existing files | Updating analysis with new findings |
| file_search_glob | Find files by pattern | Discovering relevant logs/metrics |
| file_search_content | Search file contents | Finding specific patterns/errors |
| shell_execute | Execute commands | Running diagnostic scripts |
| web_search | Search web | Finding similar issues/patterns |
| web_fetch | Fetch specific URLs | file_reading bug reports, documentation |
...
1. **Gathering evidence from logs:**
   ```
   file_search_content(pattern="ERROR|TIMEOUT", path="/var/log/app/", output_mode="content", -C=5)
   → Returns error context with surrounding lines
   ```

2. **Finding configuration issues:**
   ```
   file_read(file_path="/path/to/config.yaml")
   → Examine configuration for root cause analysis
   ```

3. **Checking metrics data:**
   ```
   shell_execute(command="curl -s 'http://prometheus:9090/api/v1/query?query=rate(errors[5m])'")
...
<constraints>
<must>Create file with file_write tool at projects/${JERRY_PROJECT}/analysis/</must>
