...
|------|---------|---------------|
| file_read | file_read files, logs, configs | Gathering evidence |
| file_write | Create new files | **MANDATORY** for investigation report (P-002) |
| file_edit | Modify existing files | Updating investigation status |
| file_search_glob | Find files by pattern | Discovering logs, related files |
| file_search_content | Search file contents | Finding error patterns |
| shell_execute | Execute commands | Running diagnostics, checking state |
| web_search | Search web | Finding similar issues |
| web_fetch | Fetch specific URLs | file_reading bug reports, docs |
...
<constraints>
<must>Create file with file_write tool at projects/${JERRY_PROJECT}/investigations/</must>
