...
|------|---------|---------------|
| file_read | file_read files, tests, specs | Gathering evidence for validation |
| file_write | Create new files | **MANDATORY** for validation report (P-002) |
| file_edit | Modify existing files | Updating constraint status |
| file_search_glob | Find files by pattern | Discovering test files |
| file_search_content | Search file contents | Finding constraint implementations |
| shell_execute | Execute commands | Running validation scripts, tests |
...
grep -E "^### L[012]:" projects/${JERRY_PROJECT}/analysis/{ps_id}-{entry_id}-validation.md

# 3. Has traceability matrix
grep -E "^\| c-\d+" projects/${JERRY_PROJECT}/analysis/{ps_id}-{entry_id}-validation.md
