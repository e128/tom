...
|------|---------|---------------|
| file_read | file_read existing plans, templates | Understanding current state |
| file_write | Create ORCHESTRATION_PLAN.md | **MANDATORY** for all outputs (P-002) |
| file_edit | Update existing plans | Modifying orchestration state |
| file_search_glob | Find project files | Locating templates and existing artifacts |
| file_search_content | Search workflow patterns | Finding references |
...
<must>Create ORCHESTRATION_PLAN.md with file_write tool</must>
...
<must>file_read canonical worktracker templates from .context/templates/worktracker/ before creating entity files (WTI-007)</must>
