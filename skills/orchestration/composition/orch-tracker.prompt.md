...
|------|---------|---------------|
| file_read | file_read ORCHESTRATION.yaml | **MANDATORY** before any update |
| file_write | Create checkpoint files | Persistence for recovery |
| file_edit | Update ORCHESTRATION.yaml | **PRIMARY** - atomic state updates |
| file_search_glob | Find orchestration files | Locating current state |
| file_search_content | Search for agent status | Finding specific entries |
| shell_execute | Execute git commands | Version control for state |
...
1. file_read `workflow.id` from ORCHESTRATION.yaml
2. file_read `pipelines.{pipeline}.short_alias` for the agent's pipeline
...
<constraints>
<must>file_read current ORCHESTRATION.yaml FIRST</must>
...
| Event | Action | Tool |
|-------|--------|------|
| Phase completion checkpoint | Store phase results + metrics | `mcp__memory-keeper__context_save` |
| Session resume | Retrieve last checkpoint | `mcp__memory-keeper__context_get` |
| Quality gate results | Store QG scores for cross-reference | `mcp__memory-keeper__context_save` |
| Cross-phase lookup | Search prior phase context | `mcp__memory-keeper__context_search` |
