...
| Tool | Purpose |
|------|---------|
| file_read | file_read canonical transcript JSON from ts-parser |
| file_write | Output extraction report (MANDATORY per P-002) |
...
input:
  ...
  chunks_path: chunks/
  constraints:
    - file_read index.json first for metadata
...
index.json ────────────────────┐
    │                          │
    ▼                          ▼
file_read metadata              Plan extraction
    │                          │
    └──────────────┬───────────┘
                   │
                   ▼
...
for_each_chunk:
  1. Load chunk file:
     - file_read chunks/chunk-NNN.json
...
output:
  - file_write unified extraction-report.json
...
### agent_delegate-to-Strategy Mapping

| Extraction agent_delegate | Recommended Strategy | Rationale |
|-----------------|---------------------|-----------|
...
1. **file_write extraction report** to the specified output path
