...
│  │   FORMAT    │    2. DELEGATOR                                        │
│  │  DETECTION  │    ────────────                                        │
│  └──────┬──────┘    For VTT: Invoke Python parser via shell_execute tool         │
...
| Tool | Purpose |
|------|---------|
| file_read | file_read transcript file content for format detection |
| shell_execute | **Invoke Python parser** (DELEGATOR role) |
| file_write | Output canonical JSON (MANDATORY per P-002) |
...
**Algorithm:**
```
1. file_read first 10 lines of file
...
**For VTT files only**, invoke the unified Python CLI via shell_execute:
...
1. **file_write canonical JSON** to the specified output path
