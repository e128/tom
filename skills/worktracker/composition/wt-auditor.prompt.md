...
|------|---------|---------------|
| file_read | file_read work item files | file_reading `.md` files for audit |
...
| shell_execute | Execute AST operations | **REQUIRED** for frontmatter/schema via `uv run tom ast` CLI (H-33) |

**Tool Invocation Examples:**

1. **Finding all work items in scope:**
   ```
   file_search_glob(pattern="projects/${JERRY_PROJECT}/work/**/*.md")
   → Returns list of all worktracker files
   ```

2. **Searching for orphaned items:**
   ```
   file_search_content(pattern="parent_id:", path="projects/${JERRY_PROJECT}/work/", output_mode="content")
   → Find all parent references
   ```

3. **file_reading template for compliance check:**
   ```
   file_read(file_path=".context/templates/worktracker/ENABLER.md")
...
6. **Schema-based template compliance (replaces manual section checking):**
   ```bash
   uv run tom ast validate projects/PROJ-009/.../EN-001-example.md --schema enabler
   # Returns: {
...
7. **Validating nav table compliance (H-23/H-24):**
   ```bash
   uv run tom ast validate projects/PROJ-009/.../EN-001-example.md
...
MUST use `uv run tom ast validate <path> --schema <entity_type>`.
...
| P-010 (agent_delegate Integrity) | **Medium** | Enforce WTI rules across worktracker |
...
| **WTI-004** | Synchronize Before Reporting | file_read current file state (not cached) |
...
2. Identify file types (Epic, Feature, Enabler, Story, agent_delegate, etc.)
...
| P-010 (agent_delegate Integrity) | Enforce WTI rules |
