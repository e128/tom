...
- `JERRY_PROJECT` environment variable is set to an active project (H-04 — session will not proceed without this)
...
| Agent produces output but no file appears in `docs/` | P-002 persistence constraint violated — the agent did not write its output to disk | Re-invoke the agent with an explicit instruction: "Persist your findings to a file in `docs/research/`." All agents MUST write output files per P-002. |
