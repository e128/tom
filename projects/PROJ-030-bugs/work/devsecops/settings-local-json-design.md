...
5. **Keep the hooks block.** The hooks in `settings.local.json` (WebSearch/WebFetch auto-allow) serve a different purpose than the hooks in the backup `settings.json.bkup` (PreToolUse/PostToolUse/SubagentStop/SessionStart). The committed `settings.json` has NO hooks block. The local hooks provide developer-specific auto-approval for web tools. Retain them.
