...
| SR-007 | Governance Auto-Escalation | MUST | T | (T-1) Perform an action that touches `.context/rules/`; verify auto-escalation to C3. (T-2) Perform an action that touches `docs/governance/TOM_CONSTITUTION.md`; verify auto-escalation to C4. | Auto-escalation rules (AE-001 through AE-006) fire correctly. | PASS: Both escalation tests produce correct criticality. FAIL: Either escalation fails to trigger. |
