...
**Criticality: C4 (Critical).** AE-003 establishes C3 as the automatic floor for new ADRs. This ADR exceeds C3 because it establishes an irreversible governance baseline that will govern all future agent development -- meeting the C4 definition ("irreversible, architecture/governance/public"). Once baselined, any modification triggers AE-004 (auto-C4). Initial C4 classification is therefore warranted.
...
These findings confirm that the schema is structurally sound but needs enum expansion and `additionalProperties` relaxation before production deployment. **Note:** These six revision recommendations (F-01 through F-06 above) must be resolved before the schema is baselined at v1.0.0. The Migration Path includes a Phase 0 prerequisite step to complete this schema finalization before Phase 1 validation begins. The Migration Path Phase 1 (validation-only) will then surface remaining violations systematically across all 37 agents.
...
### Phase 0: Schema Finalization (Prerequisite)

1. Revise the JSON Schema per Section 2 backward-compatibility recommendations (F-01 through F-06) before executing Phase 1 validation. These recommendations address enum expansion, `additionalProperties` relaxation, MCP tool name reconciliation, `output.levels` format support, and `output` requirement level.
2. Publish the revised schema as v1.0.0 only after all six recommendations are resolved.
3. Duration: 1 sprint.

### Phase 1: Validation-Only (Non-Breaking)

1. Deploy finalized JSON Schema to `docs/schemas/agent-definition-v1.0.0.json`.
