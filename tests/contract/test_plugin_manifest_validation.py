...
class TestActualMarketplaceManifest:
    ...
    def test_actual_marketplace_json_has_keywords(
        ...
        tom_plugin = next((p for p in plugins if p.get("name") == "tom"), None)

        # Assert
        assert tom_plugin is not None, "tom plugin not found"
        assert "keywords" in tom_plugin, "tom plugin missing keywords"
        assert isinstance(tom_plugin["keywords"], list), "keywords must be an array"
        assert len(tom_plugin["keywords"]) > 0, "keywords array is empty"
