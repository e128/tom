...
Tests cover:
    - AC-ST002-1: Extract all key-value pairs from a Tom entity blockquote frontmatter
...
Test Categories:
    - Happy path: Valid Tom entity frontmatter, extraction, modification, addition
...
# ---------------------------------------------------------------------------
# Test fixtures: canonical Tom entity source strings
# ---------------------------------------------------------------------------
...
class TestExtractFrontmatterFunction:
    ...
    def test_returns_blockquote_frontmatter_instance(self) -> None:
        ...
        doc = TomDocument.parse(STORY_FRONTMATTER)
...
class TestBlockquoteFrontmatterExtract:
    ...
    def test_extract_spike_entity(self) -> None:
        ...
        doc = TomDocument.parse(SPIKE_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
    ...
    def test_extract_enabler_entity(self) -> None:
        ...
        doc = TomDocument.parse(ENABLER_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
    ...
    def test_extract_numeric_values_as_string(self) -> None:
        ...
        doc = TomDocument.parse(NUMERIC_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
        assert fm.get("Effort") == "5"
    ...
    def test_extract_inline_markdown_in_values(self) -> None:
        ...
        doc = TomDocument.parse(INLINE_MARKDOWN_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
    ...
    def test_extract_field_positions_are_set(self) -> None:
        ...
        doc = TomDocument.parse(MINIMAL_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
        field = fm._fields[0]
...
class TestBlockquoteFrontmatterSet:
    ...
    def test_set_returns_tom_document(self) -> None:
        """set() returns a TomDocument instance."""
        doc = TomDocument.parse(STORY_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
        result = fm.set("Status", "in-progress")
        assert isinstance(result, TomDocument)
    ...
    def test_set_does_not_mutate_original_doc(self) -> None:
        """set() does not modify the original TomDocument."""
        doc = TomDocument.parse(STORY_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
...
class TestBlockquoteFrontmatterAdd:
    ...
    def test_add_returns_tom_document(self) -> None:
        """add() returns a TomDocument instance."""
        doc = TomDocument.parse(STORY_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
    ...
    def test_add_does_not_mutate_original_doc(self) -> None:
        """add() does not modify the original TomDocument."""
        doc = TomDocument.parse(STORY_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
...
class TestBlockquoteFrontmatterEdgeCases:
    ...
    def test_two_placeholder_fields(self) -> None:
        ...
        doc = TomDocument.parse(PLACEHOLDER_FRONTMATTER)
        fm = BlockquoteFrontmatter.extract(doc)
        assert fm.get("Due") == "--"
        assert fm.get("Completed") == "--"
    ...
    def test_multiline_blockquote_only_extracts_key_value_lines(self) -> None:
        ...
        doc = TomDocument.parse(MULTILINE_BLOCKQUOTE)
        fm = BlockquoteFrontmatter.extract(doc)
